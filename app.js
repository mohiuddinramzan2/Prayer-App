...
const state = {
  ...
  currentTab: "prayer",
  azanPlayed: {},   // track which prayers azan played today
};

// ── Azan ─────────────────────────────────────────────
const AzanPlayer = {
  fajr:   new Audio('assets/azan_fajr.mp3'),
  normal: new Audio('assets/azan_normal.mp3'),

  play(prayer) {
    const audio = prayer === 'Fajr' ? this.fajr : this.normal;
    audio.currentTime = 0;
    audio.volume = 1.0;
    audio.play().catch(err => console.warn('আজান:', err));
  },
  stop() {
    this.fajr.pause();
    this.normal.pause();
    this.fajr.currentTime = 0;
...
function renderSpecialPanel() {
  ...
  // Jumma = Dhuhr time (typically 12:30-1:00 PM; we show Dhuhr time)
  const jTime  = state.times.Dhuhr;
  // Khutba ~15 min before Dhuhr
  ...
  if (isFri) {
    note.textContent = "🕌 আজ জুম'আর দিন! আল্লাহু আকবার";
...
function updateIftarCountdown() {
  ...
  el.textContent = cd || "ইফতারের সময় অতিবাহিত হয়েছে";
}
...
function undoTasbih() {
  if (Tasbih.history.length === 0) return;
...
function updateTasbihUI() {
  ...
  // Progress ring: circumference = 2π×52 ≈ 326.7
  const pct    = Tasbih.count / Tasbih.target;
